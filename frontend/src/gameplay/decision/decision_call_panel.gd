...
func _update_mute_button() -> void:
	_mute_button.text = "UNMUTE" if _muted else "MUTE"


func _update_speaker_button() -> void:
	_speaker_button.text = "SPEAKER: ON" if _speaker_on else "SPEAKER"
